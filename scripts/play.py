...
def run_play(task_id: str, cfg: PlayConfig):
    ...
    if is_tracking_task:
        ...
        if cfg.motion_file is not None and Path(cfg.motion_file).exists():
            ...
            raise ValueError("Tracking tasks in dummy mode require: --motion-file /path/to/motion.npz")
    log_dir: Path | None = None
    resume_path: Path | None = None
    if TRAINED_MODE:
        if cfg.checkpoint_file is None:
            raise ValueError("`--checkpoint_file` is required for trained agent mode.")
    ...
    if DUMMY_MODE:
        ...
        print(f"[INFO]: Exported policy: {export_dir / 'policy.onnx'}")
